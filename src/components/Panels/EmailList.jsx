...
const EmailList = observer(class EmailList extends Component {
    ...
    loadEmails() {
        ...
        this.props.storage.setEmails([]);

        if (this.props.storage.selectedFilter !== undefined &&
            this.props.storage.selectedFilter.id !== '_default') {

            fetch(apiEndpoint, {
                method: 'POST',
                body: JSON.stringify(this.props.storage.selectedFilter.config),
                headers: new Headers({
                    'Content-Type': 'application/json'
                })
            }).then(response => response.json())
                .then(json => {
                    this.props.storage.setEmails(json)
                });
        } else {
            fetch(apiEndpoint)
                .then(response => response.json())
    ...
    render() {
        let emails = this.props.storage.emails.map((email) =>
        ...
        return (
            <div
                className="col-3 bg-light border border-dark border-top-0 border-bottom-0 border-left-0 full-height email-list">
                {emails}
            </div>
        )
    }
